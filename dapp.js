...
const nftSAddress = '0x4AA0b324322BA2B22561BAF847d00D7dc18fFc48'
...
const ssGetStake = document.getElementById('ss-get-value-stake')

ssGetStake.onclick = async () => {


  console.log('teste get value stake')
  ...
  for (let i = 1; i<= numberOfTokenCreated; i++){
 	...
 	$("body #nftImgStake").html("");
	$(document).ready(function(){	
        var nftMinted = document.getElementById('nftImgStake') 
		for (var i = 1; i <= numberOfTokenbyOwner; i++) {  
			var  img = "<img src =" + imagem[i-1] + " id= "+" myid"+i+"/> <br>"; 
			var  label1 = "<label for="+"name"+">NFT Name: " + name[i-1] + "</label> <br>";
			var  label2 = "<label for="+"name"+">NFT Price: " + price[i-1] + "</label> <br>";
			 
			$("body #nftImgStake").append(label1);
			$("body #nftImgStake").append(label2);
			$("body #nftImgStake").append(img);
			console.log(img);
